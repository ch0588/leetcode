...
public:
    ...
    void dfs(string &s,int len,string cur,int pos,int rl,int rr,int ol){
        ...
        if(rn[pos]<ol) return; //cut branch, not matched left > reamin right num
        
        if(s[pos]=='('){
            dfs(s,len,cur+"(",pos+1,rl,rr,ol+1); //don't remove this left
            if(rl>0)
                dfs(s,len,cur,pos+1,rl-1,rr,ol); //remove this left
        }
        else if(s[pos]==')'){
            if(ol>0)
                dfs(s,len,cur+")",pos+1,rl,rr,ol-1); //don't remove this right
            if(rr>0)
    ...
    vector<string> removeInvalidParentheses(string s) {
        ...
        int removedLeft = 0,removedRight = 0;
        int openLeft = 0;
        for(auto c : s){
            if(c=='(') openLeft++;
            else if(c==')'){
        ...
        removedLeft = openLeft;
        printf("%d %d\n",removedLeft,removedRight);
        dfs(s,(int)s.size(),"",0,removedLeft,removedRight,0);
        return res;
    }
};